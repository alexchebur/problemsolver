...
time_series_file = st.file_uploader(
    "Загрузите файл XLSX с рядами данных для анализа (не забудьте сформулировать основную проблему в строке запроса выше). Результаты обработки добавятся в контекст рассуждений:",
    type=["xlsx"],
...
if time_series_file is not None:
    with st.expander("🔍 Предпросмотр данных (первые 10 строк)", expanded=True):
        try:
            ...
            with st.markdown("###🔧 Диагностика данных"):
