...
STRUCTURE_PROMPT = """
Ты - профессиональный русский писатель, мастер художественной прозы в одном ряду с Толстым, Набоковым, Сорокиным, Пелевиным. На основе предоставленных пользователем данных создай подробную структуру повести.
...
- Сюжет: НЕ ОТКЛОНЯЙСЯ от полной структуры произведения {structure}
- ОБЕСПЕЧЬ ПЛАВНЫЕ ПЕРЕХОДЫ И ЛОГИЧЕСКУЮ ПРЕЕМСТВЕННОСТЬ согласно структуре {structure}
- Развивай сюжетные линии согласно общей структуре {structure} и {chapter_details}, НЕ выдумывай текст главы как законченного произведения, текст главы должен быть написан как ЧАСТЬ произведения, соответствующая его структуре и названию главы.
- Используй установленные характеры персонажей последовательно
- Развивай конфликты, заложенные в структуре
- Используй указанные локации и предметы согласно общей структуре
- Естественные диалоги, соответствующие характерам персонажей
...
Напиши полный текст главы {chapter_number}, начиная непосредственно с повествования.
"""
...
4. Несоответствия имеющейся структуре сюжета и синопсису глав.
5. Повторяющиеся местоимения 'он', 'она' заменяй на чередование местоимений и имен конкретных персонажей.
...
def generate_chapter(genre, setting, alias, structure, chapter_number, chapter_details, word_count):
    """Генерирует отдельную главу повести (фиксированная температура 0.3)"""
    try:
        alias_text = f"Дополнительная идея пользователя: {alias}" if alias else ""
...
def literary_edit_chapter(genre, setting, alias, structure, chapter_number, chapter_text):
...
def main():
    ...
    # Шаг 2: Генерация повести
        ...
        st.info("📝 Главы генерируются на основе структуры без учета текста предыдущих глав")
        ...
        # Генерируем главу БЕЗ учета предыдущего контекста
    ...
    **Процесс создания:**
    ...
    - Каждая глава генерируется независимо на основе структуры
