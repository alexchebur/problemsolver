...
CHAPTER_PROMPT = """
Ты - профессиональный писатель в одном ряду с Сорокиным, Толстым, Набоковым, Пелевиным. Напиши главу повести на основе предоставленной структуры и ПРЕДЫДУЩЕЙ ГЛАВЫ.
...
## ПРЕДЫДУЩАЯ ГЛАВА {previous_chapter_number}:
...
- УЧТИ ВСЕ СОБЫТИЯ И ДИАЛОГИ ИЗ ПРЕДЫДУЩЕЙ ГЛАВЫ
- ОБЕСПЕЧЬ ПЛАВНЫЕ ПЕРЕХОДЫ И ЛОГИЧЕСКУЮ ПРЕЕМСТВЕННОСТЬ С ПРЕДЫДУЩЕЙ ГЛАВОЙ
- НЕ ДОПУСКАЙ ПРЕВРАЩЕНИЯ ГЛАВЫ В САМОСТОЯТЕЛЬНЫЙ РАССКАЗ СО СВОЕЙ КУЛЬМИНАЦИЕЙ, РАЗВЯЗКОЙ, ФИНАЛОМ, ЕСЛИ ЭТО НЕ СООТВЕТСТВУЕТ СТРУКТУРЕ СЮЖЕТА {structure}
- Развивай сюжетные линии, начатые в предыдущей главе
...
Напиши полный текст главы {chapter_number}, начиная непосредственно с повествования, КОТОРОЕ ЛОГИЧЕСКИ ВЫТЕКАЕТ ИЗ ПРЕДЫДУЩЕЙ ГЛАВЫ.
"""
...
## ПРЕДЫДУЩАЯ ГЛАВА (для контекста):
{previous_chapter_text}
...
- ОБЕСПЕЧЬ ПРЕЕМСТВЕННОСТЬ С ПРЕДЫДУЩЕЙ ГЛАВОЙ
...
def generate_chapter(genre, setting, alias, structure, previous_chapter_text, chapter_number, chapter_details, word_count):
    """Генерирует отдельную главу повести с учетом предыдущей главы"""
    try:
        alias_text = f"Дополнительная идея пользователя: {alias}" if alias else ""
        
        # Формируем текст предыдущей главы для контекста
        previous_chapter_number = chapter_number - 1
        if previous_chapter_number == 0:
            previous_chapter_text = "Это первая глава, предыдущих глав нет."
        else:
            previous_chapter_text = f"Текст предыдущей главы {previous_chapter_number}:\n{previous_chapter_text}"
        ...
        prompt = CHAPTER_PROMPT.format(
            ...
            alias_text=alias_text,
            structure=structure,
            previous_chapter_number=previous_chapter_number,
...
def literary_edit_chapter(genre, setting, alias, structure, previous_chapter_text, chapter_number, chapter_text):
...
def main():
    ...
    if 'previous_chapter_text' not in st.session_state:
        st.session_state.previous_chapter_text = ""  # Текст предыдущей главы для контекста
    ...
    # Шаг 2: Генерация повести
        ...
        st.info("📝 Каждая глава генерируется с учетом текста предыдущей главы для обеспечения последовательности сюжета")
        ...
        # Генерируем главу с учетом предыдущей главы
        chapter_text = generate_chapter(
            genre, setting, alias, st.session_state.structure,
            st.session_state.previous_chapter_text,  # Передаем текст предыдущей главы
            chapter_num + 1, 
            chapter_details, 
            words_per_chapter
        )
        ...
        if chapter_text:
            ...
            with st.spinner(f"✏️ Редактирую главу {chapter_num + 1}..."):
                edited_chapter_text = literary_edit_chapter(
                    genre, setting, alias, st.session_state.structure,
                    st.session_state.previous_chapter_text,  # Контекст предыдущей главы
                    chapter_num + 1,
                    chapter_text
                )
            ...
            # Сохраняем текст текущей главы как предыдущую для следующей итерации
            st.session_state.previous_chapter_text = edited_chapter_text
    ...
    **Процесс создания:**
    ...
    - Каждая глава генерируется с учетом предыдущей для целостности
