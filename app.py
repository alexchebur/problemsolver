...
REASONING_STEPS = [
    "Первая часть отчета: постановка проблемы и расширение контекста. Проанализируй запрос пользователя {query} и на его основе сформулируй подразумеваемую пользователем проблему. Придумай пять сходных по смыслу концепций-тезисов из смежных запросу пользователя контекстов, используй метод Tree of Thoughts. Составь пошаговую цепочку рассуждений, направленную на решение проблемы. Опиши всю цепочку рассуждений, воспроизведя внутренний диалог размышляюшего человека (НЕ УКАЗЫВАЙ, что воспроизводишь внутренний диалог)",
    "Вторая часть отчета: гипотезы о причинах и варианты решения разными методами. Сформулируй наиболее вероятные причины и предпосылки проблемы на основе контекста и цепочки рассуждений из {context}, сформулируй варианты решения проблемы с применением методов познания из {sys_prompt}",
