...
def apply_cognitive_method(method_name, context):
    ...
    - Для процессов и потоков пишите код визуализаций в формате Mermaid версии >1.0.1, СТРОГО код, начинающийся с пометки вида ```mermaid, СТРОГО вертикальной ориентации direction TD - top-down или TB - top-bottom, без горизонтальной ориентации LR или RL
    - Требования к коду Mermaid: 
    
    1. Всегда начинайте с `graph TD` или `graph TB`
    2. Используйте простые идентификаторы: A, B, C и т.д.
    3. Максимум 10 узлов на диаграмму
    4. Текст в узлах должен быть кратким (3-5 слов)
    5. Используйте только базовые формы: 
    - Прямоугольник: A["Текст"]
    - Круг: B(("Текст"))
    - Ромб: C{{"Текст"}}
    6. Пример корректной диаграммы:
    ```mermaid
    graph TD
        A["Начало"] --> B{{"Решение?"}}
        B -->|Да| C["Путь 1"]
