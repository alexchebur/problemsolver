...
2. Проведите рассуждения (внутренний диалог из вытекающих друг из друга 10 шагов каждый вида: вопрос-ответ-сомнения-возражения-вывод) СТРОГО в формате каждого шага:
   ...
   [взвешенный вывод]
   ...
3. Создайте список из 5 поисковых запросов длиной не более одного предложения каждый
4. Избегайте запрещенного контента
...
7. Никогда не оставляйте пустых позиций между литерами блоков, заполняйте стрелками, например: J --> K --> L
8. Пример корректной диаграммы:
```mermaid
graph TD
    A["Начало"] --> B{{"Решение?"}};
    B -->|Да| C["Путь 1"];
...
Требования:
Учтите данные из уточняющего поиска
